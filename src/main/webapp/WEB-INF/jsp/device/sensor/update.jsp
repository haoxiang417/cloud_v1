...
<c:forEach items="${dsList }" var="item" varStatus="vs">
	<p style="overflow: hidden" id="p${vs.index}">
    	...
    	<select id="sType_${vs.index }" name="sType" style="float: left; margin-left:5px;" onchange="changeType('${vs.index }')">
    	...
    	<select id="sNum_${vs.index }" name="sNum" style="float: left; margin-left:5px; ${item.type eq '4' || item.type eq '5' ? 'display:none' : ''}">
    	...
    	<a href="javascript:cleanSensor('${vs.index}')" style="float: left;">删除</a>
    </p>
</c:forEach>